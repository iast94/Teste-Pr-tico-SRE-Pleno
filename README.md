...
* **Helmfile para Orquestração**: Foi utilizado o Helmfile com template GO `helmfile.yaml.gotmpl` para gerar um hash de rollout a partir dos arquivos yaml, para acionar reinicializações (Prometheus, Grafana e Aplicação) somente quando a configuração for realmente alterada.
* **Gatilhos de Atualização no Prometheus**: Foi configurada a injeção dinâmica de podMetadata via Helmfile utilizando um hash consolidado do objeto .Values. Essa abordagem garante que qualquer alteração nos parâmetros (seja via arquivo, CLI ou variáveis de ambiente) resulte em um Rolling Update automático dos pods, mantendo a consistência da observabilidade.
